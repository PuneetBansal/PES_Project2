...
CU_ASSERT_NOT_EQUAL((cb_init(tx_buffer,10)),0);

tx_buffer->buffer=malloc(sizeof(uint8_t)*10);

CU_ASSERT_EQUAL((cb_init(tx_buffer,10)),0);
...
*/
   ...
   if ((NULL == CU_add_test(pSuite, "test of cb_init()", test_cb_init)) ||(NULL == CU_add_test(pSuite, "test of cb_add()", test_cb_add)) )
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
